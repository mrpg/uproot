...
async def test_missing_attribute_returns_default():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "nonexistent")
...
async def test_non_callable_attribute_returns_value():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "attr_value")
    assert result == "test_value"

    result = await ensure_awaitable(optional_call, obj, "number")
    assert result == 42


async def test_sync_callable_with_no_args():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "sync_method")
    assert result == 3  # 1 + 2 default


async def test_sync_callable_with_kwargs():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "sync_method", x=5, y=10)
    assert result == 15


async def test_async_callable_with_no_args():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "async_method")
    assert result == 2  # 1 * 2 default


async def test_async_callable_with_kwargs():
    obj = MockObject()
    result = await ensure_awaitable(optional_call, obj, "async_method", x=3, y=4)
    assert result == 12
...
async def test_callable_with_exception():
    ...
    with pytest.raises(ValueError, match="test error"):
        await ensure_awaitable(optional_call, obj, "failing")
...
async def test_async_callable_with_exception():
    ...
    with pytest.raises(RuntimeError, match="async error"):
        await ensure_awaitable(optional_call, obj, "failing_async")
...
async def test_lambda_function():
    ...
    result = await ensure_awaitable(optional_call, obj, "lambda_attr", x=5)
    assert result == 10
